...
//  프로필 카드(완성도) / 지원 현황 / 채용 관리 / 이력서 관리 / 문의함 / 내 레슨
// ─────────────────────────────────────────────────────────────
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: insets.bottom + 40, maxWidth: 560, width: '100%' as const, alignSelf: 'center' as const, gap: spacing.md }}>
          ...
          {profile ? (
            ...
            >
              ...
              <View style={{ alignItems: 'flex-end' }}>
                ...
                <Text style={[styles.completionLabel, { color: colors.textLight }]}>이력서 완성도</Text>
              </View>
            </Pressable>
          ) : (
            <Pressable
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.profileMeta, { color: colors.textLight }]}>프로필을 만들면 공고 지원과 이력서 관리가 열려요</Text>
              </View>
              <Ionicons name="chevron-forward" size={16} color={colors.textLight} />
            </Pressable>
          )}
          ...
          <View style={[styles.section, { backgroundColor: colors.surface, borderColor: colors.border }, shadows.sm]}>
            ...
            <MenuRow icon="document-text-outline" label="이력서 관리" hint="경력·자격증·입상 기록을 문서로 관리해요" onPress={() => router.push('/coach/resumes' as never)} />
          ...
          {/* 공통 */}
          <View style={[styles.section, { backgroundColor: colors.surface, borderColor: colors.border }, shadows.sm]}>
            <MenuRow icon="chatbubbles-outline" label="레슨 문의함" badge={chatUnread} onPress={() => router.push('/coach/inbox' as never)} />
          </View>
        </ScrollView>
      )}
    </View>
