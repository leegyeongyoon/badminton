...
export default function ApplicantDetail() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setApp(found);
      if (found) setCoach(await coachApi.get(found.coachProfileId));
    } catch {
      /* noop */
    } finally {
      setLoading(false);
  ...
  const setStatus = async (status: string, offer?: OfferTerms) => {
    if (!postId || !appId || !app || busy) return;
    const act = async () => {
      setBusy(true);
      try {
        const { threadId } = await coachJobApi.setApplicationStatus(postId, appId, status, offer);
        if (status === 'INTERVIEW') {
          ...
          <View style={styles.actionRow}>
            {app.status === 'APPLIED' && (
              <>
                ...
                <Pressable onPress={() => setStatus('INTERVIEW')} disabled={busy} style={({ pressed }) => [styles.primaryBtn, { backgroundColor: colors.primary, flex: 2 }, (pressed || busy) && { opacity: 0.85 }]}>
                  {busy ? <ActivityIndicator color="#fff" /> : (
                    <>
