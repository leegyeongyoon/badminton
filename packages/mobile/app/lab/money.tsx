...
export default function LabMoney() {
  ...
  useEffect(() => {
    adminStatsApi.getClubs().then((cs) => {
      setClubs(cs);
    ...
    try {
      if (tab === 'settle') {
        const [d, ss] = await Promise.all([
          labApi.getSettlement(clubId, period),
          labApi.getSessions(clubId, period).catch(() => [] as LabSessionRow[]),
        ]);
        setData(d);
        setSessions(ss);
        ...
        const [g, apps] = await Promise.all([
          labApi.getGuests(clubId, period),
          labApi.getGuestApplications(clubId).catch(() => [] as LabGuestApplicationRow[]),
        ]);
        setGuests(g);
        setApplications(apps);
      } else {
        const c = await labApi.getDuesConfig(clubId);
  ...
  const togglePaid = async (userId: string, paid: boolean, amount: number) => {
    ...
    setBusyId(userId);
    try {
      if (paid) await labApi.unmarkPaid(clubId, userId, period);
      else await labApi.markPaid(clubId, userId, period, amount);
      await load();
    } finally {
      setBusyId(null);
    }
  ...
  const saveRental = async (sessionId: string) => {
    ...
    try {
      await labApi.setRentalCost(sessionId, raw ? parseInt(raw, 10) : null);
      await load();
    } catch { /* noop */ }
  };
  const toggleApplicationPaid = async (a: LabGuestApplicationRow) => {
    ...
    setBusyId(a.id);
    try {
      await labApi.updateGuestApplication(a.id, { feePaid: !a.feePaid });
      await load();
    } finally {
      setBusyId(null);
    }
  ...
  const toggleGuestPaid = async (r: LabGuestRow) => {
    ...
    try {
      await labApi.setGuestFeePaid(r.checkInId, !r.feePaid);
      await load();
    } finally {
      setBusyId(null);
    }
  ...
  const saveConfig = async () => {
    ...
    setSaving(true);
    try {
      await labApi.setDuesConfig(clubId, {
